...
- Chatting context (messages, files, channels)
- Calls context (handles all the media, real-time audio/video/screen sharing logic/features)
- Notifications context
- User and authentication context (users and their settings)
- Search context (search for servers (overview page), channels, users, messages, files... Anything)
- Communities / servers context? Seeing potential future features (regrouping servers into communities ? Etc.), I think it's rightly a context.
Are servers their own context?? Or are they part of the chatting context?

How do these contexts organize around teams, features, and how they'd interact with each other?
I'm confused for channels. Chat and calls need to interact with channels, it's part of their endpoints, it's their parent. Messages as well as calls belong to a specific channel, but does that mean neither one of these two should "own"/manage channels? Whose context are channels a part of?

[cols="1,1,2"]
|===
|Service name |Goal and provided features |Corresponding user stories

|Authentication / User
|Authenticate users(, authenticate external (and internal) requests)(?).
|
...
|Chat
|Enable users to send messages and share files.
|
...
|Voice/Video
|Enable users to share voice/video feeds with minimal latency.
|

|Authorization
|Manage (CRUD, apply policies) user roles and permissions across services.
|

|User
|Manages (CRUD) user-related data (pfp, username/email/password..., preferences, etc.)
|
