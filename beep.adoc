...
- Notifications context (push notifications to devices. At least that would need a message queuing system, I believe?)
...
->Should user and auth be one service, or two services? Is auth, login, register etc part of users? Managed by the same team? Same group of features? Same goal? OR should auth be a separate service, and also be used to authenticate other stuff like bots, and other requests?
...
image::images/fig2.use-case-current/beep-uml-use-case-diagram-light.svg[Component diagram showing Beep's architecture separated into microservices.]
